...
export async function createAnalysis({ userId, formData }: CreateAnalysisParams) {
  try {
    ...
    // Determine analysis type and payment status
    let analysisType: 'standard' | 'premium' | 'whitespace' = 'standard';
    let paymentStatus: 'unpaid' | 'pending' | 'paid' = 'unpaid';
    let amountPaid = null;

    if (formData.pricingTier === 'standard') {
      analysisType = 'standard';
      paymentStatus = 'pending'; // Will be 'paid' after payment
      amountPaid = 2500;
    } else if (formData.pricingTier === 'premium') {
    ...
    // TODO: When Stripe/Resend keys are available:
    // 1. If paid tier, create Stripe checkout session
    ...
    console.log('TODO: Start analysis workflow');
